...
pub enum LayerShellEvent<'a, T, Message> {
    ...
    /// A `wl_output` (monitor) was removed by the compositor, carrying the name
    /// it had. The mirror of [`LayerShellEvent::OutputConnected`]. The surfaces
    /// bound to it are also torn down and reported as ordinary window-close
    /// events; this event additionally lets a program react by output name.
